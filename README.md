...
| A | 職稱 | 經理／內場組長／PT／支援 |
| B | 姓名 | 身分對照的主鍵 |
| C | **Email** | 員工首次 Google 登入綁定後自動寫入；也可由店長手動預填。**這一欄預設是隱藏的**（系統用來反查身分，不是給人看的）。隱藏不影響讀寫，要看的話選 B、D 兩欄右鍵「取消隱藏欄」。舊分頁可以執行 `hideEmailColumn()` 一次隱藏 |
...
每週固定時間，對「下週（一〜日）還有格子沒填」的人**各發一則個人私訊**。
只要七天裡有任何一格空著就會收到（填「排休」也算填了），全員都填完就不發。
同事之間看不到誰還沒交。

- 排程（開關／星期／時段）在**管理頁的「03 LINE 催繳提醒」**由店長自己改，
  存成 Script Properties，實際靠 Apps Script 的時間驅動觸發器跑。
  預設是**週五 12 時台**（週次觸發器只精確到「時段」）。
- 私訊要有 LINE 的 userId。員工**從 LINE 的選單開一次排班頁（LIFF）**就會
  自動對應，不用做任何登入操作。對照表在試算表的 `LINE連携` 分頁。
  - 一次都沒開過的人送不出去，管理頁會標成「無法提醒」，請店長另外告知。
  - ⚠ 沒把官方帳號加為好友的人也收不到，而且 **LINE 仍然回 200**，
    所以程式無法察覺。試送之後請向對方確認真的有收到。
  - LIFF／LINE Login 頻道**必須跟 Messaging API 頻道在同一個 provider 底下**，
    否則拿到的 userId 對不上。
  - 備援：在群組裡打「綁定 你的姓名」也可以（沒用過 LIFF 時用）。
...
- 班表是「確認設定」一天一天寫進去的，所以**寫入時只計數、不發訊息**，
  ...
  所以連按兩次也不會重複。
- 收件帳號在**管理頁的「04 提交通知」**按「連結我的 LINE」設定
...
| LINE リマインドの曜日・時刻 | 不要（管理頁の「03 LINE 催繳提醒」で変更） |
